...
const generando = slug => fs.existsSync(path.join(dirAuto(slug), 'generando.lock'));
...
function generarAhora(slug) {
  ...
  fs.writeFileSync(path.join(d, 'generando.lock'), String(Date.now()));
  ...
  hijo.on('exit', code => {
    fs.closeSync(logF);
    fs.rmSync(path.join(d, 'generando.lock'), { force: true });
    let mensaje = '';
...
async function auto(req, u, accion, slug, ctx, c) {
  ...
  if (accion === 'horas') { // horario (horas de Lima en que corre el workflow)
    ...
    if (!horas.length || !horas.every(h => HORA.test(h))) return out(400, { error: 'Horario no válido (usa HH:MM, hora de Lima)' });
  ...
  if (accion === 'auto-feed') {
    if (!c.ftpUrl) return out(409, { error: 'Falta FTP_URL (la dirección pública de la carpeta FTP_DIR)' });
    ...
    for (const f of b.filas) {
      ...
      if (Object.hasOwn(productos, f.id)) return out(400, { error: 'id repetido: ' + String(f.id).slice(0, 40) });
    ...
    fs.writeFileSync(path.join(d, 'feed.csv'), csvAuto(filas, c.ftpUrl + '/' + slug));
    fs.writeFileSync(path.join(d, 'estado.nuevo.json'), JSON.stringify({ version: 1, slug, actualizado: ahora, productos, retirados }));
    const res = { productos: filas.length, nuevas, reusadas: filas.length - nuevas, retiradas: Object.keys(retirados).length, atrasadas: Number(b.atrasadas) || 0, leidos: Number(b.leidos) || 0, fuera: b.fuera || {} };
    fin({ ok: true, ...res });
    return out(200, res);
  }
...
async function publicar(req, res, u, ctx) {
  ...
  if (req.method === 'GET' && accion === 'estado') {
    ...
    return out(200, { github: listo(c), repo: c.repo, rama: c.rama, base: c.base, feeds, recetas: recetas().map(s => infoReceta(s, c)).filter(Boolean), ftp_url: c.ftpUrl });
  }
  ...
  const ACCIONES_AUTO = ['receta', 'progreso', 'auto-fin', 'auto-img', 'auto-feed', 'activo', 'horas', 'borrar', 'generar'];
  const slug = u.searchParams.get('slug') || '';
